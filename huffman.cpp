...
void huffman::createarray() {
    for (int i = 0; i < 128; i++) { // max value 0 to 127
...
// Conversion function string to the decimal 

int huffman::binToDec(string inStr) {
    return stoi(inStr, nullptr, 2);
}

// decimal to binary conversion
string huffman::decToBin(int inNum) {
    string temp = "", res = ""; // initialize 
    while (inNum > 0) { // convert the value to binary but its reversed
        temp += (inNum % 2 + '0');
        inNum /= 2;
    }
    // make it 8 bit by appending the 0s to the right
    res.append(8 - temp.length(), '0');
    for (int i = temp.length() - 1; i >= 0; i--) { // reverse the temp with padded zeros
        res += temp[i];
    }
    return res;
}

// tree, insert at the end of leaf a_code char while binary is path
void huffman::buildTree(char a_code, string& path) {
    Node* curr = root;
    for (int i = 0; i < path.length(); i++) {
        if (path[i] == '0') {
            if (curr->left == NULL) { // if code is 0 then go to the left and insert new node there
                curr->left = new Node();
            }
            curr = curr->left;
        }
            ...
            if (curr->right == NULL) {
                curr->right = new Node(); // else go to right and insert there
            }
            curr = curr->right;
        }
    }
    curr->data = a_code; // change the data of node to the a_code which can be "A"etc
}


// count frequencies & take note of characters with least no of freq
...
// extract two nodes with smaller frequencies & merge them
...
// open the file, read data, take length and pad 0 + length+ 0
//
